...
def load_model():
    ...
    if model_path is None:
        raise FileNotFoundError(
            "Place model.keras or mnist_cnn_model.keras beside app.py."
        )
...
st.markdown(
    ...
    .bar-row {
        display: grid;
        grid-template-columns: 18px 1fr 49px;
        gap: 10px;
        align-items: center;
        margin: 10px 0;
        font: 500 12px 'DM Mono', monospace;
...
st.markdown(
    '<p class="eyebrow">Live neural vision</p>',
    unsafe_allow_html=True
)

st.markdown(
    "<h1>Draw a digit.<br>Watch it think.</h1>",
    unsafe_allow_html=True
)
...
with left:
    ...
    st.markdown(
        '<p class="hint">Draw anywhere inside the square</p>',
        unsafe_allow_html=True
    )
